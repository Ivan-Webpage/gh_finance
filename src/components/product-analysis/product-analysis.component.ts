...
export class ProductAnalysisComponent implements AfterViewInit {
  ...
  private readonly analysisCategoryStructure: ReadonlyArray<{ group: string; subCategories: ReadonlyArray<string> }> = [
    ...
    { group: '酒水', subCategories: ['調酒', '紅酒/白酒', '啤酒', '威士忌', '太魯閣威士忌'] },
  ...
  selectedMarginStartYear = signal<number>(this.now.getFullYear());
  selectedMarginStartMonth = signal<number>(1);
  selectedMarginEndYear = signal<number>(this.now.getFullYear());
  selectedMarginEndMonth = signal<number>(this.now.getMonth() + 1);
  ...
  async applyAnalysisFilter(): Promise<void> {
    ...
    try {
      ...
      if (response.data && response.data.length > 0) {
        // 後端 /api/product-sales 已經算好依分類加總的結果（含太魯閣威士忌調整邏輯），
        // 直接使用 response.categories，不在前端重新分組加總一次。
        this.productSalesData.set((response.categories || []) as CategorySalesSummary[]);
        // 若 canvas 已存在，立即刷新圖表；否則交給 ViewChild setter 在 canvas 出現時繪製
        if (this.monthlyTrendChartRef?.nativeElement) {
          queueMicrotask(() => this.createMonthlyTrendChartOnly());
        }
      } else {
        this.productSalesError.set('查無資料，請確認查詢條件（僅支援 2024-2025 年度）');
      }
    } catch (error) {
  ...
  resetMarginFilter(): void {
    this.selectedMarginStartYear.set(this.now.getFullYear());
    this.selectedMarginStartMonth.set(1);
    this.selectedMarginEndYear.set(this.now.getFullYear());
    this.selectedMarginEndMonth.set(this.now.getMonth() + 1);
    void this.applyMarginFilter();
  }
