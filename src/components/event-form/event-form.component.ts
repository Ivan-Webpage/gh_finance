...
export class EventFormComponent {
  ...
  readonly websiteSyncOptions: { value: 'moonMusic' | 'venueClosure' | ''; label: string }[] = [
    { value: '', label: '不同步' },
    { value: 'moonMusic', label: '漫霧與音樂之約（自動產生內容）' },
    { value: 'venueClosure', label: '包場公告（用活動名稱當標題、備註當內文）' },
  ];
  ...
  async onSubmit(): Promise<void> {
    ...
    const websiteSyncType = this.eventForm.get('websiteSyncType')?.value || '';
    if (!this.isEditMode() && websiteSyncType) {
      const eventName = this.eventForm.get('name')?.value || '';
      const confirmed = confirm(`這筆活動送出後會自動在官網新增一篇「${eventName}」文章並公開發佈，確定要送出嗎？`);
      if (!confirmed) return;
    }
