...
export class ShareholderRecordsComponent {
  ...
  yearFilter = signal<number>(this.currentDate.getFullYear());
  monthFilter = signal<string>(String(this.currentDate.getMonth() + 1).padStart(2, '0'));
  ...
  yearOptions = Array.from({ length: 5 }, (_, i) => this.currentDate.getFullYear() + 2 - i);
  ...
  filteredMeetings = computed(() => {
    ...
    let meetings = this.sortedMeetings();
  ...
  resetFilters(): void {
    this.yearFilter.set(this.currentDate.getFullYear());
    this.monthFilter.set(String(this.currentDate.getMonth() + 1).padStart(2, '0'));
