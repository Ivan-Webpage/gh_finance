...
export class AccountsPayableComponent {
  ...
  // --- 已付清篩選（發生日期／還款期限／債權人），預設當年當月 ---
  private readonly currentYearMonth = (() => {
    const now = new Date();
    return `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
  })();

  paidFilterEntryMonth = signal<string>(this.currentYearMonth);
  paidFilterDueMonth = signal<string>(this.currentYearMonth);
  paidFilterCreditor = signal<string>('');

  paidCreditorOptions = computed(() => {
    const names = this.rows()
      .filter(r => this.toNumber(r.remaining_amount) <= 0)
      .map(r => r.creditor)
  ...
  paidRows = computed(() => {
    const entryMonth = this.paidFilterEntryMonth();
    const dueMonth = this.paidFilterDueMonth();
    const creditor = this.paidFilterCreditor();

    return this.rows().filter(r => {
      if (this.toNumber(r.remaining_amount) > 0) return false;
      if (entryMonth && String(r.entry_date || '').slice(0, 7) !== entryMonth) return false;
      if (dueMonth && String(r.due_date || '').slice(0, 7) !== dueMonth) return false;
      if (creditor && r.creditor !== creditor) return false;
      return true;
    });
  });

  resetPaidFilters(): void {
    this.paidFilterEntryMonth.set(this.currentYearMonth);
    this.paidFilterDueMonth.set(this.currentYearMonth);
    this.paidFilterCreditor.set('');
  }
