...
export class DashboardComponent {
  ...
  dashboardAnnouncements = signal<Array<{ title: string; date: string; finalDecision: string }>>([]);
  expandedAnnouncementIndex = signal<number | null>(null);

  toggleAnnouncement(index: number): void {
    this.expandedAnnouncementIndex.update(current => (current === index ? null : index));
  }
    ...
    try {
      ...
      const mapped = (response.data as any[])
        .map(item => {
          ...
          return { title, date, finalDecision };
        })
  ...
  async loadData(startDate?: string, endDate?: string): Promise<void> {
    ...
    try {
      ...
      const metricsResponse = await this.apiService.getDashboardMetrics({
        startDate: start,
        endDate: end,
        yearlyStartDate: this.calculateMonthsBack(12),
      });

      if (metricsResponse.success && metricsResponse.data) {
        this.dashboardData.set(metricsResponse.data.dashboardData);
        this.dailyRevenueTrend.set(metricsResponse.data.dailyRevenueTrend);
        this.monthlyPerformance.set(metricsResponse.data.monthlyPerformance);
      } else {
        this.errorMessage.set(metricsResponse.error || '無法取得儀表板核心指標');
      }

      await this.loadPayablesData();
    } catch (error) {
      console.error('載入財務數據時發生異常:', error);
      const errorMsg = error instanceof Error ? error.message : '無法取得儀表板核心指標';
  ...
  private async loadPayablesData(): Promise<void> {
    const [
      ...
      this.apiService.getDashboardPayablesLedgerSummary(),
      this.apiService.getAccountsPayables(),
    ]);
    ...
    const payablesSummary = payablesSummaryResult.status === 'fulfilled' && payablesSummaryResult.value?.success
      ? payablesSummaryResult.value.data
      : null;

    // 儀表板「應付帳款」卡片顯示：以尚未付清的剩餘欠款為準（與 /accounts-payable 表格一致）
    const accountsPayableRemainingTotal =
      accountsPayableSummaryResult.status === 'fulfilled' && accountsPayableSummaryResult.value?.success
        ? (accountsPayableSummaryResult.value.data || []).reduce((sum: number, row: any) => {
            const remaining = this.toNumber(row?.remaining_amount);
            return remaining > 0 ? sum + remaining : sum;
          }, 0)
        : this.toNumber(payablesSummary?.accountsPayableTotal);
    ...
    this.payablesData.set({
      ...
      cigarPurchaseTotal: this.toNumber(payablesSummary?.cigarPurchaseTotal),
      renovationTotal: this.toNumber(payablesSummary?.renovationTotal),
      accountsPayableMonthly: payablesSummary?.accountsPayableMonthly || [],
      cigarPurchaseMonthly: payablesSummary?.cigarPurchaseMonthly || [],
    });
  }
