...
export class LedgerComponent {
  ...
  selectedItemSubAccounts = computed(() => {
    const selectedItem = this.ledgerForm.get('item_group')?.value as string | null;
    const accounts = this.glAccounts();
  ...
  constructor() {
    ...
    effect(() => {
      ...
      const categories = this.availableCategories(); // 強制重新計算
  ...
  formatCurrency(value: number | null | undefined): string {
    ...
    const formatted = absValue.toLocaleString('zh-TW', { style: 'currency', currency: 'TWD', minimumFractionDigits: 0, maximumFractionDigits: 0 });
