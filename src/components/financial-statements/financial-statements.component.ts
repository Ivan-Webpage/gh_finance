...
import { MonthlyIncomeStatementReport, MonthlyBalanceSheetReport, AnnualSummaryReport, MonthlyDetailReport, ExpenseBreakdown, RevenueBreakdown, UnpaidBreakdown, Tax401Report, MonthlyRevenueTable, MonthlyRevenueRowKey } from '../../models/financial.model';
import { Chart } from 'chart.js/auto';

type Statement = 'summary' | 'income' | 'balance' | 'monthlyRevenue' | 'tax401';
...
function emptyTax401Form(): Tax401FormState {
  return {
    report_id: null,
    period_label: '',
    period_start_date: '',
...
export class FinancialStatementsComponent {
  ...
  // --- 月營收表 State（沿用損益表/資產負債表同一組年度＋季度篩選）---
  monthlyRevenueTable = signal<MonthlyRevenueTable | null>(null);
  actualCostRowHint(key: MonthlyRevenueRowKey): string {
    return ACTUAL_COST_ROW_HINTS[key] || '';
  }

  // --- 401報表 State ---
  tax401Reports = signal<Tax401Report[]>([]);
  isTax401ModalOpen = signal(false);
  showTax401Uploader = signal(false);
  tax401Form = signal<Tax401FormState>(emptyTax401Form());
  tax401Error = signal<string | null>(null);
  tax401Saving = signal(false);
  tax401AiScanning = signal(false);
  tax401SelectedFile = signal<File | null>(null);

  tax401InputTotal = computed(() => this.tax401Reports().reduce((sum, r) => sum + Number(r.input_amount || 0), 0));
  tax401OutputTotal = computed(() => this.tax401Reports().reduce((sum, r) => sum + Number(r.output_amount || 0), 0));
  tax401TaxTotal = computed(() => this.tax401Reports().reduce((sum, r) => sum + Number(r.tax_amount || 0), 0));

  constructor() {
    this.applySummaryFilter();
    this.applyFilter();
    this.loadTax401Reports();
  }

  async loadMonthlyRevenueTable(): Promise<void> {
    try {
      const response = await this.apiService.getMonthlyRevenueTable(this.selectedYear(), this.selectedQuarter());
      if (response.success && response.data) {
        this.monthlyRevenueTable.set(response.data);
      } else {
        this.errorMessage.set(response.error || '無法取得月營收表資料');
      }
    } catch (error) {
      console.error('Error loading monthly revenue table:', error);
      this.errorMessage.set('無法載入月營收表數據。');
    }
  }
  ...
  private normalizeBalanceSheet(report: MonthlyBalanceSheetReport): MonthlyBalanceSheetReport {
    ...
    // 負債及業主權益總計沒有自己的科目明細（items 恆為空陣列），是後端算好的
    // 「負債小計 + 業主權益小計」衍生值，不能套用 normalizeSection()（會把 items
    // 加總出 0 蓋掉後端正確算好的數字），這裡只做字串轉數字，不重新計算。
    const normalizeDerivedTotal = (section: any) => {
      const monthlyTotals: { [month: string]: number } = {};
      Object.entries(section?.monthlyTotals || {}).forEach(([month, amount]: any) => {
        monthlyTotals[month] = this.convertToNumber(amount);
      });
      return {
        ...section,
        monthlyTotals,
        grandTotal: this.convertToNumber(section?.grandTotal),
      };
    };
    ...
    return {
      ...
      totalLiabilitiesAndEquity: normalizeDerivedTotal(report.totalLiabilitiesAndEquity)
    };
  }
  ...
  async applyFilter(): Promise<void> {
    ...
    try {
      ...
      // 取得月營收表資料（沿用同一組年度＋季度篩選）
      await this.loadMonthlyRevenueTable();
    } catch (error) {
  ...
  /**
   * 損益表/資產負債表/月營收表用：負數以括號顯示（不重複顯示負號），正數維持原樣
   */
  formatSignedCurrency(value: number | null | undefined): string {
    const num = Number(value) || 0;
    const formatted = Math.abs(num).toLocaleString('zh-TW', { style: 'currency', currency: 'TWD', minimumFractionDigits: 0, maximumFractionDigits: 0 });
  ...
  async loadTax401Reports(): Promise<void> {
    try {
      const response = await this.apiService.getTax401Reports();
      if (response.success && response.data) {
        this.tax401Reports.set(response.data);
      } else {
        this.tax401Error.set(response.error || '報表期別失敗');
      }
    } catch (error) {
      console.error('Error loading tax 401 reports:', error);
      this.tax401Error.set('報表期別失敗，請稍後重試');
    }
  }

  openTax401Modal(report: Tax401Report | null = null): void {
    this.tax401Error.set(null);
    this.tax401SelectedFile.set(null);
    this.showTax401Uploader.set(false);
    if (report) {
      this.tax401Form.set({
        report_id: report.report_id,
        period_label: report.period_label,
        period_start_date: report.period_start_date,
        output_amount: report.output_amount,
        input_amount: report.input_amount,
        tax_amount: report.tax_amount,
        remark: report.remark || '',
      });
    } else {
      this.tax401Form.set(emptyTax401Form());
    }
    this.isTax401ModalOpen.set(true);
  }

  openTax401AiImportModal(): void {
    this.tax401Error.set(null);
    this.tax401Form.set(emptyTax401Form());
    this.showTax401Uploader.set(true);
    this.isTax401ModalOpen.set(true);
  }

  closeTax401Modal(): void {
    this.isTax401ModalOpen.set(false);
  }

  updateTax401FormField<K extends keyof Tax401FormState>(field: K, value: Tax401FormState[K]): void {
    this.tax401Form.update(f => ({ ...f, [field]: value }));
  }
  ...
  onTax401FileSelected(event: Event): void {
    const input = event.target as HTMLInputElement;
    const file = input.files?.[0];
    this.tax401SelectedFile.set(file || null);
    this.tax401Error.set(null);
  }

  async startTax401AiScan(): Promise<void> {
    const file = this.tax401SelectedFile();
    if (!file) {
      this.tax401Error.set('請先選擇檔案（圖片或 PDF）');
      return;
    }

    this.tax401AiScanning.set(true);
    this.tax401Error.set(null);

    try {
      const fileBase64 = await this.readFileAsBase64(file);
      const response = await this.apiService.scanTax401ReportAi({
        filename: file.name,
        mimeType: file.type,
        fileBase64,
      });
      ...
      if (response.success && response.data) {
        ...
        this.tax401Error.set(response.error || '辨識失敗');
      }
    } catch (error) {
      console.error('Error scanning tax 401 report:', error);
      this.tax401Error.set('辨識失敗，請稍後重試');
    } finally {
      this.tax401AiScanning.set(false);
    }
  }

  async submitTax401Report(): Promise<void> {
    const form = this.tax401Form();

    if (!form.period_label.trim() || !form.period_start_date) {
      this.tax401Error.set('請先確認必填欄位已正確填寫');
      return;
    }

    this.tax401Saving.set(true);
    this.tax401Error.set(null);
    ...
    try {
      const payload = {
        ...
        output_amount: Number(form.output_amount) || 0,
        input_amount: Number(form.input_amount) || 0,
        tax_amount: Number(form.tax_amount) || 0,
        remark: form.remark.trim() || null,
      };

      const response = form.report_id
        ? await this.apiService.updateTax401Report({ ...payload, report_id: form.report_id })
        : await this.apiService.createTax401Report(payload);

      if (response.success) {
        this.closeTax401Modal();
        await this.loadTax401Reports();
      } else {
        this.tax401Error.set(response.error || '報表期別失敗');
      }
    } catch (error) {
      console.error('Error saving tax 401 report:', error);
      this.tax401Error.set('報表期別失敗，請稍後重試');
    } finally {
      this.tax401Saving.set(false);
    }
  }

  async deleteTax401Report(report: Tax401Report): Promise<void> {
    if (!confirm(`您確定要刪除「${report.period_label}」這一期的 401 報表資料嗎？`)) return;

    try {
      const response = await this.apiService.deleteTax401Report(report.report_id);
      if (response.success) {
        await this.loadTax401Reports();
      } else {
        this.tax401Error.set(response.error || '報表期別失敗');
      ...
      console.error('Error deleting tax 401 report:', error);
      this.tax401Error.set('報表期別失敗，請稍後重試');
    }
  }
}
