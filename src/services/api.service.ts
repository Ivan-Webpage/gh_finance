...
import type { 
  ...
  RecipeComponentType,
  RecipeType,
  RecipeTopLevelCategory,
  ItemCostLink,
  ItemCostLinkProductType,
  MonthlyRevenueTable,
...
export class ApiService {
  ...
  /**
   * 儀表板核心指標：一次在 SQL 端算好當期核心指標/每日營收/近12個月月績效，
   * 取代前端逐筆分類加總最多 10000 筆流水帳資料的做法
   */
  getDashboardMetrics(params: { startDate: string; endDate: string; yearlyStartDate: string }): Promise<ApiResponse<{
    dashboardData: {
      revenue: number; expenses: number; allowance: number; purchases: number;
      beveragePurchase: number; foodPurchase: number; cigarPurchase: number;
    };
    dailyRevenueTrend: { date: string; amount: number }[];
    monthlyPerformance: { month: string; revenue: number; expenses: number; allowance: number; purchases: number }[];
  }>> {
    const httpParams = new HttpParams()
      .set('startDate', params.startDate)
      .set('endDate', params.endDate)
      .set('yearlyStartDate', params.yearlyStartDate);
    return firstValueFrom(this.http.get<ApiResponse<any>>(`${this.baseUrl}/dashboard/metrics`, { params: httpParams }));
  }

  /**
   * 儀表板「應付帳款/應付雪茄進貨/應付裝潢款」卡片彙總（依科目全歷史）
   */
  getDashboardPayablesLedgerSummary(): Promise<ApiResponse<{
    accountsPayableTotal: number;
    accountsPayableMonthly: { monthLabel: string; amount: number }[];
    cigarPurchaseTotal: number;
    cigarPurchaseMonthly: { monthLabel: string; amount: number }[];
    renovationTotal: number;
  }>> {
    return firstValueFrom(this.http.get<ApiResponse<any>>(`${this.baseUrl}/dashboard/payables-ledger-summary`));
  }
  ...
  /**
   * 取得月營收表（欄=月份、列=營收/折讓/酒水餐飲雪茄實際成本/費用/小計）
   */
  async getMonthlyRevenueTable(year: number, quarter: number | 'all'): Promise<ApiResponse<MonthlyRevenueTable>> {
    const params = new HttpParams()
      .set('type', 'monthlyRevenue')
      .set('year', String(year))
      .set('quarter', String(quarter));

    const response = await firstValueFrom(
      this.http.get<ApiResponse<MonthlyRevenueTable>>(`${this.baseUrl}/financial-statements`, { params })
    );

    if (response.success && response.data) {
      response.data.rows = response.data.rows.map(row => ({
        ...row,
        total: this.parseNumeric(row.total),
        monthlyAmounts: Object.fromEntries(
          Object.entries(row.monthlyAmounts).map(([month, amount]) => [month, this.parseNumeric(amount)])
        ),
      }));
    }
  ...
  *   - entryId: 流水帳 entry_id (可選，用於關聯查詢)
  ...
  getFuturePlans(params?: { keyword?: string; ownerEmployeeId?: number }): Promise<ApiResponse<FuturePlan[]>> {
    let httpParams = new HttpParams();
    if (params?.keyword) httpParams = httpParams.set('keyword', params.keyword);
    if (params?.ownerEmployeeId) httpParams = httpParams.set('ownerEmployeeId', String(params.ownerEmployeeId));
    return firstValueFrom(this.http.get<ApiResponse<FuturePlan[]>>(`${this.baseUrl}/collaboration/future-plans`, { params: httpParams }));
  }

  createFuturePlan(payload: { title: string; content: string; ownerEmployeeId: number }): Promise<ApiResponse<FuturePlan>> {
    return firstValueFrom(this.http.post<ApiResponse<FuturePlan>>(`${this.baseUrl}/collaboration/future-plans`, payload));
  }

  updateFuturePlan(payload: { id: number; title?: string; content?: string; ownerEmployeeId?: number }): Promise<ApiResponse<FuturePlan>> {
    return firstValueFrom(this.http.put<ApiResponse<FuturePlan>>(`${this.baseUrl}/collaboration/future-plans`, payload));
  }

  deleteFuturePlan(id: number): Promise<ApiResponse<{ id: number }>> {
    const params = new HttpParams().set('id', String(id));
    return firstValueFrom(this.http.delete<ApiResponse<{ id: number }>>(`${this.baseUrl}/collaboration/future-plans`, { params }));
  }

  generateFuturePlanTopic(data: { content: string; maxLen?: number }): Promise<ApiResponse<{ topic: string }>> {
    return firstValueFrom(
  ...
  // ========== 外帳財務 (off-book-finance) ==========

  /**
   * 取得外帳發票紀錄，year/month 皆可省略
   */
  getOffBookInvoices(params?: { year?: number; month?: number }): Promise<ApiResponse<OffBookInvoice[]>> {
    let httpParams = new HttpParams();
    ...
    return firstValueFrom(this.http.get<ApiResponse<OffBookInvoice[]>>(`${this.baseUrl}/off-book-invoices`, { params: httpParams }));
  }

  createOffBookInvoice(payload: { invoice_name: string; amount: number; invoice_date: string }): Promise<ApiResponse<OffBookInvoice>> {
    return firstValueFrom(this.http.post<ApiResponse<OffBookInvoice>>(`${this.baseUrl}/off-book-invoices`, payload));
  }

  deleteOffBookInvoice(invoiceId: number): Promise<ApiResponse<{ invoice_id: number }>> {
    const params = new HttpParams().set('id', String(invoiceId));
    return firstValueFrom(this.http.delete<ApiResponse<{ invoice_id: number }>>(`${this.baseUrl}/off-book-invoices`, { params }));
  }

  /**
   * 取得財務規劃任務，year/month 皆可省略（依 planned_complete_date 篩選）
   */
  getFinancialPlanTasks(params?: { year?: number; month?: number }): Promise<ApiResponse<FinancialPlanTask[]>> {
    let httpParams = new HttpParams();
    if (params?.year) httpParams = httpParams.set('year', String(params.year));
    if (params?.month) httpParams = httpParams.set('month', String(params.month));
    return firstValueFrom(this.http.get<ApiResponse<FinancialPlanTask[]>>(`${this.baseUrl}/financial-plan-tasks`, { params: httpParams }));
  }

  createFinancialPlanTask(payload: { period_label?: string; task_content?: string }): Promise<ApiResponse<FinancialPlanTask>> {
    return firstValueFrom(this.http.post<ApiResponse<FinancialPlanTask>>(`${this.baseUrl}/financial-plan-tasks`, payload));
  }

  updateFinancialPlanTask(payload: {
    task_id: number;
    period_label?: string | null;
    task_content?: string | null;
    status: FinancialPlanTaskStatus;
    ...
    remark?: string | null;
  }): Promise<ApiResponse<FinancialPlanTask>> {
    return firstValueFrom(this.http.put<ApiResponse<FinancialPlanTask>>(`${this.baseUrl}/financial-plan-tasks`, payload));
  }

  deleteFinancialPlanTask(taskId: number): Promise<ApiResponse<{ task_id: number }>> {
    const params = new HttpParams().set('id', String(taskId));
    return firstValueFrom(this.http.delete<ApiResponse<{ task_id: number }>>(`${this.baseUrl}/financial-plan-tasks`, { params }));
  }

  // ========== 401報表 (tax-401-reports) ==========

  getTax401Reports(): Promise<ApiResponse<Tax401Report[]>> {
    return firstValueFrom(this.http.get<ApiResponse<Tax401Report[]>>(`${this.baseUrl}/tax-401-reports`));
  }

  createTax401Report(payload: {
    period_label: string;
    ...
    tax_amount: number;
    remark?: string | null;
  }): Promise<ApiResponse<Tax401Report>> {
    return firstValueFrom(this.http.post<ApiResponse<Tax401Report>>(`${this.baseUrl}/tax-401-reports`, payload));
  }

  updateTax401Report(payload: {
    report_id: number;
    period_label: string;
    period_start_date: string;
    input_amount: number;
    ...
    return firstValueFrom(this.http.put<ApiResponse<Tax401Report>>(`${this.baseUrl}/tax-401-reports`, payload));
  }

  deleteTax401Report(reportId: number): Promise<ApiResponse<{ report_id: number }>> {
    const params = new HttpParams().set('id', String(reportId));
    return firstValueFrom(this.http.delete<ApiResponse<{ report_id: number }>>(`${this.baseUrl}/tax-401-reports`, { params }));
  }

  /**
   * AI 辨識 401 報表圖片/PDF，僅回傳辨識結果草稿，不寫入資料庫
   */
  scanTax401ReportAi(payload: { filename: string; mimeType: string; fileBase64: string }): Promise<ApiResponse<Tax401AiScanResult>> {
    return firstValueFrom(this.http.post<ApiResponse<Tax401AiScanResult>>(`${this.baseUrl}/tax-401-reports/ai-scan`, payload));
  }

  // ========== 食材採購主檔 (ingredients) ==========

  getIngredients(): Promise<ApiResponse<Ingredient[]>> {
    return firstValueFrom(this.http.get<ApiResponse<Ingredient[]>>(`${this.baseUrl}/ingredients`));
  }
  ...
  createIngredient(payload: {
    ...
    return firstValueFrom(this.http.post<ApiResponse<Ingredient>>(`${this.baseUrl}/ingredients`, payload));
  }
  ...
  updateIngredient(payload: {
    ...
    return firstValueFrom(this.http.put<ApiResponse<Ingredient>>(`${this.baseUrl}/ingredients`, payload));
  }

  deleteIngredient(id: string): Promise<ApiResponse<null>> {
    const params = new HttpParams().set('id', id);
    return firstValueFrom(this.http.delete<ApiResponse<null>>(`${this.baseUrl}/ingredients`, { params }));
  }

  // ========== 餐點食譜 (recipes) ==========

  getRecipes(recipeType?: RecipeType): Promise<ApiResponse<Recipe[]>> {
    let httpParams = new HttpParams();
    if (recipeType) httpParams = httpParams.set('recipeType', recipeType);
    return firstValueFrom(this.http.get<ApiResponse<Recipe[]>>(`${this.baseUrl}/recipes`, { params: httpParams }));
  }
  ...
  createRecipe(payload: {
    ...
    top_level_category?: RecipeTopLevelCategory | null;
    yield_quantity?: number;
    yield_unit?: string | null;
    selling_price?: number | null;
    is_active?: boolean;
    remark?: string | null;
    items?: { component_type: RecipeComponentType; component_id: number; quantity: number; unit?: string | null }[];
  }): Promise<ApiResponse<{ id: string }>> {
    return firstValueFrom(this.http.post<ApiResponse<{ id: string }>>(`${this.baseUrl}/recipes`, payload));
  }
  ...
  updateRecipe(payload: {
    ...
    return firstValueFrom(this.http.put<ApiResponse<{ id: string }>>(`${this.baseUrl}/recipes`, payload));
  }

  deleteRecipe(id: string): Promise<ApiResponse<null>> {
    const params = new HttpParams().set('id', id);
    return firstValueFrom(this.http.delete<ApiResponse<null>>(`${this.baseUrl}/recipes`, { params }));
  }

  // ========== iChef 品項對應 (item-cost-links) ==========

  getItemCostLinks(): Promise<ApiResponse<ItemCostLink[]>> {
    return firstValueFrom(this.http.get<ApiResponse<ItemCostLink[]>>(`${this.baseUrl}/item-cost-links`));
  }

  setItemCostLink(payload: {
    item_uuid: string;
    ...
    return firstValueFrom(this.http.post<ApiResponse<{ id: string }>>(`${this.baseUrl}/item-cost-links`, payload));
  }

  deleteItemCostLink(itemUuid: string): Promise<ApiResponse<null>> {
    const params = new HttpParams().set('itemUuid', itemUuid);
    return firstValueFrom(this.http.delete<ApiResponse<null>>(`${this.baseUrl}/item-cost-links`, { params }));
  }

  // ========== 太魯閣威士忌銷售追蹤 (tarakou-whisky-sales) ==========

  getTarakouWhiskySales(startYear: number, endYear: number): Promise<ApiResponse<TarakouWhiskySalesData>> {
    const params = new HttpParams().set('startYear', String(startYear)).set('endYear', String(endYear));
    return firstValueFrom(this.http.get<ApiResponse<TarakouWhiskySalesData>>(`${this.baseUrl}/tarakou-whisky-sales`, { params }));
  }
  ...
  createTarakouExternalInventory(payload: {
    ...
    return firstValueFrom(this.http.post<ApiResponse<any>>(`${this.baseUrl}/tarakou-whisky-sales`, { table: 'external', payload }));
  }
  ...
  updateTarakouExternalInventory(id: number, payload: {
    ...
    return firstValueFrom(this.http.put<ApiResponse<any>>(`${this.baseUrl}/tarakou-whisky-sales`, { table: 'external', id, payload }));
  }

  deleteTarakouExternalInventory(id: number): Promise<ApiResponse<{ id: number }>> {
  ...
  createTarakouShareholderPurchase(payload: {
    ...
    return firstValueFrom(this.http.post<ApiResponse<any>>(`${this.baseUrl}/tarakou-whisky-sales`, { table: 'shareholder', payload }));
  }
  ...
  updateTarakouShareholderPurchase(id: number, payload: {
    ...
    return firstValueFrom(this.http.put<ApiResponse<any>>(`${this.baseUrl}/tarakou-whisky-sales`, { table: 'shareholder', id, payload }));
  }
