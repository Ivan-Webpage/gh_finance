...
export type RecipeTopLevelCategory = '酒水' | '餐飲';
...
export interface Recipe {
  ...
  topLevelCategory: RecipeTopLevelCategory | null;
...
export type ItemCostLinkProductType = 'wine' | 'cigar' | 'recipe';

export interface ItemCostLink {
  itemUuid: string;
  itemName: string;
  ...
  linkId: string | null;
  productType: ItemCostLinkProductType | null;
...
export type TarakouExternalOperation = '廠商入倉' | '銀河提貨' | '廠商客戶提貨' | '客人內用' | '客人外帶';

export interface TarakouExternalInventory {
  id: number;
...
export interface TarakouShareholderPurchase {
  id: number;
...
export interface TarakouMonthlySales {
  month: string;
  salesAmount: number;
}

export interface TarakouWhiskySalesData {
  startYear: number;
  endYear: number;
  monthlySales: TarakouMonthlySales[];
  externalInventory: TarakouExternalInventory[];
  shareholderPurchases: TarakouShareholderPurchase[];
  currentStockQty: number;
}

// ========== 月營收表 (financial-statements?type=monthlyRevenue) ==========

export type MonthlyRevenueRowKey =
  | 'revenue' | 'discount' | 'drinksActualCost' | 'diningActualCost' | 'cigarActualCost'
  | 'expense' | 'subtotal';

export interface MonthlyRevenueTableRow {
  key: MonthlyRevenueRowKey;
  label: string;
  monthlyAmounts: Record<string, number>;
  total: number;
}

export interface MonthlyRevenueTable {
  months: string[];
  rows: MonthlyRevenueTableRow[];
}

// ========== 401報表 (tax-401-reports) ==========

export interface Tax401Report {
  report_id: number;
  period_label: string;
...
export interface Tax401AiScanResult {
  period_label: string;
  period_start_date: string;
  output_amount: number;
  input_amount: number;
...
export interface OffBookInvoice {
  invoice_id: number;
  invoice_name: string;
  amount: number;
  invoice_date: string;
...
export type FinancialPlanTaskStatus = '未開始' | '進行中' | '已完成' | '延遲';
